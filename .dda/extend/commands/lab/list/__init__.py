...
def _recover_orphaned_stacks(app: Application, pulumi_dir: str) -> None:
    ...
    for stack in stacks:
        ...
        def _pulumi_config_get(key: str, _name: str = name, _pulumi_dir: str = pulumi_dir) -> str | None:
            r = subprocess.run(
...
def _refresh_env(app: Application, env: Any) -> bool:
    ...
    # Empty outputs mean the stack has no resources — it was fully destroyed.
    if not outputs:
        app.display_warning(f"Stack '{stack}' has no resources — removing local record and empty stack '{env.name}'.")
        LabEnvironment.load(app, env.name).delete()
        subprocess.run(
