...
func (s *packageApmInjectSuite) TestSystemdServiceRebootBrokenInjector() {
	...
	// injector. Any residual /etc/ld.so.preload entry is a dangling /run path
	// that ld.so skips. This is the core guarantee — a broken injector on disk
	// does not brick subsequent boots.
