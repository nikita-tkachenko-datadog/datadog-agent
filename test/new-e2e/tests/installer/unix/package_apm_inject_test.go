...
func (s *packageApmInjectSuite) assertLDPreloadInstrumented(libPath string) {
	...
	assert.NoError(s.T(), err)
	// OCI host instrumentation references the launcher through the reboot-safe
	// tmpfs symlink rather than its persistent path; the deb injector keeps its
	// own persistent path.
	expected := libPath
	if libPath == injectOCIPath {
		expected = injectTmpfsLauncher
	}
	assert.Contains(s.T(), string(content), expected)
}
