...
func TestNewDockerConfigReaderSurfacesDockerClientErrors(t *testing.T) {
	...
	oldNewDockerConfigClient := newDockerConfigClient
	newDockerConfigClient = func() (dockerConfigClient, error) {
		return nil, expectedErr
	}
	t.Cleanup(func() {
		newDockerConfigClient = oldNewDockerConfigClient
	})

	reader, err := newDockerConfigReader(target{runtime: RuntimeDocker, entityID: "container-id"})
